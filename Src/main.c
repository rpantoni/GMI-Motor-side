...
static void MX_TIM1_Init(void)
{
  ...
  */
  ...
  TIM_InitStruct.Autoreload = (((uint16_t)(ADV_TIM_CLK_MHz* (uint32_t)1000000u/((uint32_t)(A_PWM_FREQUENCY)))) / 2); //ORG   TIM_InitStruct.Autoreload = ((PWM_PERIOD_CYCLES) / 2);
  ...
  TIM_OC_InitStruct.OCState = LL_TIM_OCSTATE_DISABLE;
  TIM_OC_InitStruct.OCNState = LL_TIM_OCSTATE_DISABLE;
  TIM_OC_InitStruct.CompareValue = ((((uint16_t)(ADV_TIM_CLK_MHz* (uint32_t)1000000u/((uint32_t)(A_PWM_FREQUENCY)))) / 2) - (HTMIN));
